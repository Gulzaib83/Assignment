...
namespace DbOperations.Operations
{
        ...
        public class Operations : IDisposable
        {
            ...
            public ResponseObject<Ticket> GenerateTicket(Ticket ticket)
            {
                ...
                try
                {
                    ...
                    using (var context = new TicketContext())
                    {
                        ...
                        {
                            //return User object with AutoIncremented value of User_Id field (generated by SQL server) with Result Code "Success"
                            response.SetResponeData(ticket, ResultCode.Success, string.Empty);
                        }
            ...
            public ResponseObject<Boolean> DeleteUser(User user)
            {
                ...
                try
                {
                    ...
                    using (var context = new TicketContext())
                    {
                        ...
                        {
                            //return User object with AutoIncremented value of User_Id field (generated by SQL server) with Result Code "Success"
                            response.SetResponeData(true, ResultCode.Success, string.Empty);
                        }
                    }
                return response;
                }
        ...
        public ResponseObject<Boolean> DeleteTicket(Ticket ticket)
        {
            ResponseObject<Boolean> response = new ResponseObject<Boolean>();
            try
            {
                ...
                using (var context = new TicketContext())
                {
                    context.Remove<Ticket>(ticket);
                    int affectedRows = context.SaveChanges();
                    //If insert operation is successful then we check how many rows are affected by this, in this case it should always be "1" for success and 0 for failure
                    if (affectedRows > 0)
                    {
                        //Delete Ticket object from Tickets table with Result Code "Success"
                        response.SetResponeData(true, ResultCode.Success, string.Empty);
                    }
                }
                return response;
        ...
        public ResponseObject<Ticket> UpdateTicket(Ticket ticket)
        {
            ResponseObject<Ticket> response = new ResponseObject<Ticket>();
            try
            {
                ...
                using (var context = new TicketContext())
                {
                    ...
                    if (affectedRows > 0)
                    {
                        //return User object with AutoIncremented value of ticket_id field (generated by SQL server) with Result Code "Success"
                        response.SetResponeData(ticket, ResultCode.Success, string.Empty);
                    }
                }
                return response;
